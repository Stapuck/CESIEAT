...
const ShoppingCart: React.FC = () => {
    const { cartItems, removeItemFromCart, addItemToCart, getTotalPrice, updateItemQuantity } = useCart();
    const navigate = useNavigate();
    const [restaurantNames, setRestaurantNames] = useState<Record<string, string>>({}); // État pour stocker les noms des restaurants
            ...
            await Promise.all(
                uniqueRestaurantIds.map(async (id) => {
                    try {
                        ...
                        console.error(`Erreur lors de la récupération du nom du restaurant ${id}:`, error);
                        names[id] = 'Inconnu';
                    }
                })
            );
    ...
    return (
        <div className="container mx-auto py-8 px-4 min-h-screen">
            <div className="flex items-center mb-6">
                <button onClick={() => navigate(-1)} className="flex items-center text-gray-600 hover:text-amber-500 mr-4 mb-5">
            ...
            {isCartEmpty ? (
                <div className="text-center py-12">
                    <div className="flex justify-center mb-4">
                        <img src={FaShoppingBag} alt="Panier" className="w-24 h-24 text-gray-300" />
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2">
                        {cartItems.map((item: CartItem) => (
                            <div key={item.id} className="flex justify-between  items-center p-4 my-4 rounded-lg bg-white border-b last:border-0">
                                <div className="flex items-center">
                                    {item.image && <img src={item.image} className="w-16 h-16 rounded-md mr-4" />}
                                    <div>
                                        <h3 className="font-medium">{item.name}</h3>
                                        <p className="text-gray-600">
                                            Restaurant : {restaurantNames[item.restaurantId] || 'Chargement...'}
                                        </p>
                                        <p className="text-gray-600">{item.price.toFixed(2)} € / unité</p>
                                    </div>
                                </div>
                                <div className="flex items-center">
                                    <button onClick={() => {
                                    ...
                                    <button onClick={() => removeItemFromCart(item.id)} className="w-5 ml-5 rounded-2xl hover:cursor-pointer hover:transform hover:scale-110 transition-transform duration-200">
                    ...
                    <div className="bg-white rounded-lg shadow-md p-6">
                        ...
                        <p className="text-gray-600 mb-2">Total des articles : {cartItems.length}</p>
                        <p className="text-gray-600 mb-4">Prix total : <span className="font-bold">{getTotalPrice().toFixed(2)} €</span></p>
