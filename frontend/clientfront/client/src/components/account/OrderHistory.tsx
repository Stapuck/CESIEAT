...
const OrderHistory = () => {
  ...
  const fetchRestaurantDetails = async (restaurantId: string) => {
    try {
      const response = await axios.get(
        `https://cesieat.nathan-lorit.com/api/restaurateurs/${restaurantId}`
      );
  ...
  const fetchLivreurDetails = async (livreurId: string) => {
    ...
    try {
      const response = await axios.get(
        `https://cesieat.nathan-lorit.com/api/livreurs/byZitadelId/${livreurId}`
      );
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        const response = await axios.get(
          `https://cesieat.nathan-lorit.com/api/commandes/client/${clientId_Zitadel}`
        );
  ...
  const cancelOrder = async (orderId: string) => {
    ...
    try {
      ...
      await axios.put(`https://cesieat.nathan-lorit.com/api/commandes/${orderId}`, {
        status: "Annulée"
      });
