...
const OrderHistory = () => {
  ...
  const fetchRestaurantDetails = async (restaurantId: string) => {
    try {
      const response = await axios.get(`http://localhost:8080/api/restaurateurs/${restaurantId}`);
      setRestaurants(prev => ({...prev, [restaurantId]: response.data}));
    } catch (err) {
      console.error(`Erreur lors de la récupération des détails du restaurant ${restaurantId}:`, err);
    }
  };
  ...
  const fetchLivreurDetails = async (livreurId: string) => {
    if (!livreurId) return;
    try {
      const response = await axios.get(`http://localhost:8080/api/livreurs/${livreurId}`);
      setLivreurs(prev => ({...prev, [livreurId]: response.data}));
    } catch (err) {
      console.error(`Erreur lors de la récupération des détails du livreur ${livreurId}:`, err);
    }
  };
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      ...
      try {
        const response = await axios.get(`http://localhost:8080/api/commandes/client/${clientId_Zitadel}`);
  ...
  const cancelOrder = async (orderId: string) => {
    ...
    try {
      ...
      setCancelError('Impossible d\'annuler la commande. Veuillez réessayer plus tard.');
    } finally {
            ...
            {orders.map(order => (
              <li key={order._id} className="py-4 flex flex-col hover:bg-gray-50 rounded-lg p-3">
                ...
                <div className="flex justify-between text-sm text-gray-500">
                  <span>Restaurant: {restaurants[order.restaurant]?.name || 'Chargement...'}</span>
                  <span>Livreur: {order.livreur ? (livreurs[order.livreur]?.name || 'Chargement...') : 'Non assigné'}</span>
                </div>
