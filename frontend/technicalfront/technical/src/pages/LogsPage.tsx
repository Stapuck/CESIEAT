...
const LogsPage = () => {
  ...
  return (
    <div className="logs-container" style={{ padding: "20px", height: "100vh", display: "flex", flexDirection: "column" }}>
              ...
              {logs.map((log, index) => (
                <tr key={index} style={{ backgroundColor: index % 2 === 0 ? "#f9f9f9" : "white" }}>
                  <td style={tableCellStyle}>
                    <span style={getTypeStyle(log.type)}>{log.type || "N/A"}</span>
                  </td>
                  <td style={tableCellStyle}>{log.message}</td>
                  <td style={tableCellStyle}>{formatDate(log.createdAt)}</td>
                  <td style={tableCellStyle}>{log.clientId_Zitadel || "N/A"}</td>
                </tr>
              ))}
            </tbody>
          </table>
