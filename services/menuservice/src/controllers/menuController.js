...
export const getMenu = async (c) => {
    try {
        const id = c.req.param('id');
        const menu = await Menu.findById(id);
        if (!menu)
            return c.json({ message: `Menu with ID ${id} not found` }, 404);
        return c.json(menu);
    }
    catch (error) {
        const errMsg = error instanceof Error ? error.message : 'An unknown error occurred';
...
export const createMenu = async (c) => {
    try {
...
export const deleteMenu = async (c) => {
    try {
        const id = c.req.param('id');
