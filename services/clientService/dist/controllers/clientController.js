...
export const getClient = async (c) => {
    try {
...
export const editClient = async (c) => {
    try {
        const id = c.req.param('id');
        const body = await c.req.json();
...
export const pauseClient = async (c) => {
    try {
        const id = c.req.param('id');
        const client = await Client.findByIdAndUpdate(id, { isPaused: true }, { new: true, runValidators: true });
        if (!client) {
            return c.json({ message: `Client with ID ${id} not found` }, 404);
        }
